...
const _DEFAULT_ITEMS: WishlistItem[] = [
  {
    id: '1',
...
interface Wishlist1Props {
  items: WishlistItem[]
  isUpdating?: boolean
  lineIdByMerchandiseId?: Record<string, string>
  onMoveToCart?: (merchandiseId: string, lineId: string) => Promise<void>
      ...
      <div className="container max-w-6xl mx-auto px-4">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-muted-foreground">{wishlistItems.length} items saved</p>
          </div>
          {wishlistItems.length > 0 && (
            <Button variant="outline" onClick={onAddAllToCart} disabled={isUpdating}>
              ...
              <Card key={item.id} className="group gap-0 overflow-hidden p-0">
                <div className="relative">
                  <AspectRatio ratio={1} className="bg-muted">
                    <Image
                      ...
                      alt={item.name}
                      fill
                      className={cn('object-cover', !item.inStock && 'opacity-50')}
                    />
                  </AspectRatio>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      removeItem(item.id)
                      onRemove?.(lineIdByMerchandiseId[item.id])
                    }}
                    disabled={isUpdating}
                  >
                ...
                <CardContent className="p-4">
                  ...
                  <Button
                    ...
                    onClick={() => onMoveToCart?.(item.id, lineIdByMerchandiseId[item.id])}
                  >
