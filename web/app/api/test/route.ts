...
export async function GET(req: NextRequest) {
  ...
  switch (q) {
    ...
    case 'product':
      if (!handle) return NextResponse.json({ error: 'Pass ?handle=your-product-handle' }, { status: 400 })
    ...
    case 'collection':
      if (!handle) return NextResponse.json({ error: 'Pass ?handle=your-collection-handle' }, { status: 400 })
      result = await client.request(GET_PRODUCTS_IN_COLLECTION, { variables: { handle } })
      break

    case 'cart':
      if (!cartId) return NextResponse.json({ error: 'Pass ?cartId=gid://shopify/Cart/...' }, { status: 400 })
